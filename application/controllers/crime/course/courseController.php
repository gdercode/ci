<?php
	...
	//for second download_excel
	use PhpOffice\PhpSpreadsheet\IOFactory;

class CourseController extends CI_Controller										// Course controller
{
	public function __construct()										// constructor
 	{
	 	...
	 	$this->load->model('eexam/Course_model','courseManager');		// for model with an other name
	 	$this->load->model('eexam/eexam_model','eexamManager');		// for user model with an other name

	 	$this->session->set_userdata('courseCreatePermit', '70');


	 	$needed = $this->session->userdata('coursepagePermit');
...
//-------------------------------------------------------------------------------------------------------------------------------
	...
	private function check_course_page_permit($need)
	{
		...
		{
			redirect('eexam/eexamController/logout_c');										// go back to logout controller 
		}
		elseif ($permit<$needed)										// if permit session is there but less than the needed
		{
			redirect('eexam/eexamController/home_c');								// go back to homepage controller 	
		}
	}
...
//-------------------------------------------------------------------------------------------------------------------------------
	...
	private function check_course_enrolment_permit($courseID)
	{
		...
		$connection = $this->courseManager->get_user_course_connection($user_id, $module_id);  // get User and course connection
...
//--------------------------------------------------------------------------------------------------------------------------------
	...
	private function welcome($welcome_error) 											// private function for course page
	{
		...
		$data['all_course']=$this->courseManager->get_all_course(); 	
		if (empty($data['all_course']))
		{
			$data['error'] = 'No course found. ';
		}
		$this->load->view('eexam/pages/course/course_page',$data);			     		// load a course page with data
...
//-----------------------------------------------------------------------------------------------------------------------------------
	...
	public function course_page_c($moduleID)
	{
		...
		$courseData = $this->courseManager->get_course_id($module_id);
		...
		$allExams = $this->courseManager->get_all_course_exams($module_id);  // get  all exam of this course
		if (empty($allExams))
		{
			$data['examError'] = "No Exam Found";
		}

		$allQuestions = $this->courseManager->get_all_course_questions($module_id);  // get  all question of this course
		...
		$this->load->view('eexam/pages/course/course_exams_page',$data);
		return;
	}
...
//-----------------------------------------------------------------------------------------------------------------------------------
	...
	{
		$module_id = htmlspecialchars($moduleID);
		$data['courseData'] = $this->courseManager->get_course_id($module_id);
		...
		$allUserConnection = $this->courseManager->get_all_user_course_connection($module_id);  // get User and course connection
		...
		$this->load->view('eexam/pages/course/course_participants_page',$data);
		return;
	}
...
//-----------------------------------------------------------------------------------------------------------------------------------
	...
	public function enrol_users_page_c($moduleID)
	{
		...
		$this->load->view('eexam/pages/course/course_enrolment_page',$data);
		return;
	}
...
//-----------------------------------------------------------------------------------------------------------------------------------
	...
	public function course_brouse_participant_c($moduleID)
	{
		...
		$data['the_user'] = $this->eexamManager->get_user_permit($username);  						// get all of User by username
...
//-----------------------------------------------------------------------------------------------------------------------------------
	...
	public function course_enrol_participant_c($moduleID, $userID)
	{
		...
		if($this->check_course_create_permit() == FALSE OR empty($data['courseData']))
		{
     	...
     	// check the existance of username
		$this_user= $this->eexamManager->get_user_id($user_id);  						// get a User by id
		...
		{
			$userConnection= $this->courseManager->get_user_course_connection($user_id, $module_id);  // get User and course connection
			// insert connection

			if (empty($userConnection))
			{
				$this->courseManager->insert_user_course_connection($user_id, $module_id); 
...
//-----------------------------------------------------------------------------------------------------------------------------------
	...
	public function user_course_remove_connection_c($moduleID, $userID)
	{
		...
		$userConnection= $this->courseManager->get_user_course_connection($user_id, $module_id); 
		...
		if(empty( $this_user ) OR empty($userConnection))		// no user existance, then we have to give back a message
		{
			...
			// insert connection
		$this->courseManager->remove_user_course_connection($user_id, $module_id); 
		$data['error']="User removed successfully";
		$data['allUsers'] = $this->courseManager->get_all_user_course_connection($module_id); // get all User and course connection
...
//-----------------------------------------------------------------------------------------------------------------------------
	...
	public function creat_exam_c($moduleID)
	{
		...
		if ($userPermit < $createPermit ) 
		{
			...
			$this->load->view('eexam/pages/course/course_page',$data);
			return;
		}

		$this->load->view('eexam/pages/course/create_exam_page',$data);
		return;
	}
...
//---------------------------------------------------------------------------------------------------------------------------------
	...
	public function update_exam_c( $moduleID )
	{
        ...
        $examData =  $this->courseManager->get_exam_id($exam_id);  						// get a exam by id
		if (empty($examData) ) 
        {
			// insert exam
 			$this->courseManager->insert_exam($exam_title,$exam_description,$exam_open_date,$exam_open_time,$exam_close_date,$exam_close_time,$exam_grade,$exam_pass_grade,$module_id); 
 			$data['error']="Exam Created successfully";
 		}
 		else
 		{
 			//update a exam
			$this->courseManager->update_exam($exam_id,$exam_title,$exam_description,$exam_open_date,$exam_open_time,$exam_close_date,$exam_close_time,$exam_grade,$exam_pass_grade,$module_id); 
			$data['error']="Exam updated successfully"; 						// set a success message
		}

		$data['allExams'] = $this->courseManager->get_all_course_exams($module_id);  // get  all exam of this course
		if (empty($data['allExams']))
		{
			$data['examError'] = "No Exam Found";
		}

		$data['allQuestions'] = $this->courseManager->get_all_course_questions($module_id);  // get  all question of this course
...
//----------------------------------------------------------------------------------------------------------------------------
	...
	public function delete_exam_c($examID, $moduleID)
	{
        ...
        {
        	$this->courseManager->delete_exam($exam_id); 							// delete a exam
...
//------------------------------------------------------------------------------------------------------------------------------
	...
	public function edit_exam_c($examID, $moduleID)
	{
        ...
        {
        	$data['courseData'] = $this->courseManager->get_course_id($module_id);
        	$data['examData'] =  $this->courseManager->get_exam_id($exam_id);  						// get a exam by id
		}
		$this->load->view('eexam/pages/course/create_exam_page',$data);
...
//*****************************************************************************************************************************
	...
	private function wait_the_start($examID, $moduleID)
	{
		...
		$courseData = $this->courseManager->get_course_id($module_id);
		$examData =  $this->courseManager->get_exam_id($exam_id); 
		...
		if ($now < $startTimestamp) 
		{
			...
			$this->load->view('eexam/pages/course/error_messages_page',$data);
			return;
		}
...
//----------------------------------------------------------------------------------------------------------------------------
	...
	public function exam_assign_question_c($examID, $moduleID)
	{
		...
		$allQuestions = $this->courseManager->get_all_exam_questions_connection($exam_id);
		if (empty($allQuestions))
		{
		...
		else
		{
			foreach($allQuestions as $key => $question)
			{
				...
				$data['allQuestions'] [ $question['question_id']]['choices'] = $this->courseManager->get_all_question_answers($question['question_id']);  // get  all choices of this question

				$user_answwer =  $this->courseManager->get_student_exam_connection($user_id, $exam_id, $question['question_id']); // the answer of user
			...
			if ($now > $endTimestamp) 
			{
				...
				$allStudentExamAnswers =  $this->courseManager->get_all_student_exam_answers($user_id, $exam_id);
			...
			{
				$data['submition'] =  $this->courseManager->get_user_attemption($user_id, $exam_id); 
			}
		}
		
		$this->load->view('eexam/pages/course/exam_assign_question_page',$data);
...
//----------------------------------------------------------------------------------------------------------------------------
	...
	public function add_question_c($examID, $moduleID)
	{
		$exam_id = htmlspecialchars($examID);
		$module_id = htmlspecialchars($moduleID);
		$data['examData'] = $this->courseManager->get_exam_id($exam_id);
		$data['courseData'] = $this->courseManager->get_course_id($module_id);
		...
		$data['allCourseQuestions'] = $this->courseManager->get_all_course_questions($module_id);  // get  all question of this course
		if (empty($data['allCourseQuestions']))
		{
			$data['questionError'] = "No Question Found";
		}
		$this->load->view('eexam/pages/course/add_question_page',$data);
		return;
	}
...
//---------------------------------------------------------------------------------------------------------------------------
	...
	public function exam_connect_question_c()
	{
		...
		$this_exam= $this->courseManager->get_exam_id($exam_id);  						// get a exam by id
		$this_question= $this->courseManager->get_question_id($question_id);  			// get a queestion by id
		$courseData = $this->courseManager->get_course_id($module_id);
		...
		if ($this->form_validation->run() == FALSE) 												// if validation fail
	    {
	    	$data['allCourseQuestions'] = $this->courseManager->get_all_course_questions($module_id); 
			if (empty($data['allCourseQuestions']))
			{
				$data['questionError'] = "No Question Found";
			}
		...
		$examQuestionConnection= $this->courseManager->get_exam_question_connection($exam_id, $question_id);  
		// insert connection
		if (empty($examQuestionConnection))
		{
			$this->courseManager->insert_question_exam_connection($question_id, $exam_id, $question_grade); 
...
public function  exam_remove_question_connection_c($moduleID, $questionID, $examID)
{
	...
	$this_exam= $this->courseManager->get_exam_id($exam_id);  						// get a exam by id
	$this_question= $this->courseManager->get_question_id($question_id);  			// get a queestion by id
	$examQuestionConnection= $this->courseManager->get_exam_question_connection($exam_id, $question_id);  // get exam and question connection
	...
	$this->courseManager->remove_exam_question_connection($exam_id, $question_id);
	$this->courseManager->remove_enswered_exam_question_connection($exam_id, $question_id); 
...
private function ansering($moduleID,$questionID,$examID)
{
	...
	$data['allAnswers'] = $this->courseManager->get_all_question_answers($question_id);  // get  all answer of this question
	...
	$user 	 = $this->session->userdata('user_details');
	$user_id = $user['user_id'];
	$data['StudentExamQuestionAnswer'] =  $this->courseManager->get_student_exam_connection($user_id, $exam_id, $question_id);

	$data['submition'] =  $this->courseManager->get_user_attemption($user_id, $exam_id);
	// Display page
	if ($this_question['question_category'] == 'essay' OR $this_question['question_category'] == 'short_answer')
	{
		$this->load->view('eexam/pages/course/essay_or_short_answer_page',$data);
		return;
	}
	else
	{
		$this->load->view('eexam/pages/course/exam_answer_question_page',$data);		
		return;
	}
...
public function  insert_exam_answer_question_c()
{
	...
	$courseData = $this->courseManager->get_course_id($module_id);
	$this_exam = $this->courseManager->get_exam_id($exam_id);
	$questionData = $this->courseManager->get_question_id($question_id);  // get a queestion by id
	...
	if($this->check_course_create_permit() == FALSE AND $now > $endTimestamp )
	{
		...
		return;
				    	...
				    	//  for questiongrade
	$question_in_exam = $this->courseManager->get_exam_question_connection($exam_id, $question_id);
	...
	{
    	$question_with_right_answer= $this->courseManager->get_question_with_right_answer($question_id);
	...
	$userExamConnection= $this->courseManager->get_student_exam_connection($user_id ,$exam_id, $question_id); 
	
	// insert connection
	if (empty($userExamConnection))
	{
		$this->courseManager->insert_student_exam_connection($user_id, $exam_id,$question_id, $answer_title ,$answer_grade); 
		return;
	}
	else
	{
		$this->courseManager->update_student_exam_connection($user_id, $exam_id,$question_id, $answer_title ,$answer_grade); 
		return;
	}
...
public function  exam_report_c($examID, $moduleID)
{
	...
	$allStudentExamAnswers =  $this->courseManager->get_exam_all_students_with_answers($exam_id);
	...
	if (!empty($allStudentExamAnswers)) 
	{
		foreach($allStudentExamAnswers as $row)
		{
			...
			$data['student_report'][$row['user_id']]['exam_monitoring'] = $this->courseManager->get_user_attemption($row['user_id'] ,$exam_id);

		}
	}
	$data['courseData'] = $courseData;
	$data['examData'] =  $examData;
	$this->load->view('eexam/pages/course/exam_report_page',$data);
...
public function  exam_grade_manual_page_c($user_id, $moduleID, $questionID, $examID)
{
	...
	if ($this_question['question_category'] == 'essay' OR $this_question['question_category'] == 'short_answer')
	{
		$this->load->view('eexam/pages/course/essay_or_short_answer_grade_manual_page',$data);
		return;
	}
	else
	{
		$this->load->view('eexam/pages/course/exam_grade_manual_page',$data);
		return;
	}
...
public function  update_grade_manual_c()
{
    ...
    if ($this->form_validation->run() == FALSE OR $answer_grade>$question_grade) 		// if validation fail
    {
    	$data['courseData']=$courseData;
    	$data['examData']=$this_exam;
    	$data['allAnswers'] = $this->courseManager->get_all_question_answers($question_id);
	...
	// update grade
	$this->courseManager->update_manual_grade_connection($user_id, $exam_id,$question_id,$answer_grade);
...
public function  insert_user_attemption()
{
	...
	// get user and exam connection
	$user_exam_attemption= $this->courseManager->get_user_attemption($user_id ,$exam_id); 
	
	// insert connection
	if (empty($user_exam_attemption))
	{
		$this->courseManager->insert_user_attemption($user_id, $exam_id); 
	...
	else
	{
		...
		$attemption++;
		$this->courseManager->update_number_of_attemption($user_id, $exam_id, $attemption);
		return;
	}
}
//--------------------------------------------------------------------------------------------------------------
...
public function  updateFocusLost_c()
{
	...
	else
	{
		...
		$page_focus_lost++;
		$this->courseManager->update_number_of_page_focus_lost($user_id, $exam_id, $page_focus_lost);
...
public function  updateEyeFocusLost_c()
{
	...
	else
	{
		...
		$eye_focus_lost++;
		$this->courseManager->update_number_of_eye_focus_lost($user_id, $exam_id, $eye_focus_lost);
...
public function  update_submition_c()
{
	...
	else
	{
		...
		{
			$submition = 'submit';
			$this->courseManager->update_submition($user_id, $exam_id, $submition);
			$this->welcome('Exam Submitted successfully');
			return;
		}
	}
...
//-------------------------------------------------------------------------------------------------------------------------------------------
	...
	public function creat_question_c($moduleID)
	{
		...
		if ($userPermit >= $createPermit ) 
		{
			$this->load->view('eexam/pages/course/create_question_page',$data);
			return;
		}
		else
		{
...
//------------------------------------------------------------------------------------------------------------------------------------
	...
	public function creat_question_category_c($moduleID, $categ)
	{
		...
		if ($userPermit >= $createPermit ) 
		{
			$this->load->view('eexam/pages/course/create_question_category_page',$data);
			return;
		}
		else
		{
...
//------------------------------------------------------------------------------------------------------------------------------------
	...
	public function update_question_c($categ, $moduleID )
	{
		...
		{	
			$questionData =  $this->courseManager->get_question_id($question_id);  						// get a question by id
			
			if (empty($questionData)) 
	        {
				// insert question
				$this->courseManager->insert_question($question_title,$question_category,$module_id); 
				$data['error']="Question Created successfully";
			}
			else
			{
				//update a question
				$this->courseManager->update_question($question_id,$question_title,$question_category,$module_id); 
...
//----------------------------------------------------------------------------------------------------------------------------------------
	...
	public function delete_question_c($questionID, $moduleID)
	{
        ...
        {
        	$this->courseManager->delete_question($question_id); 							// delete a question
...
//-------------------------------------------------------------------------------------------------------------------------
	...
	public function edit_question_c($questionID, $moduleID)
	{
        ...
        {
        	$data['courseData'] = $this->courseManager->get_course_id($module_id);
        	$data['questionData'] =  $this->courseManager->get_question_id($question_id);  						// get a question by id
		}
		$this->load->view('eexam/pages/course/create_question_category_page',$data);
		return;
	}
...
//------------------------------------------------------------------------------------------------------------------------------
	...
	public function question_assign_answer_c($questionID, $moduleID)
	{
		...
		$this->load->view('eexam/pages/course/question_assign_answer_page',$data);
		return;
	}
//-----------------------------------------------------------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------------------
	...
	public function creat_answer_c($questionID, $moduleID)
	{
		$question_id = htmlspecialchars($questionID);
		$module_id = htmlspecialchars($moduleID);
		$data['questionData'] = $this->courseManager->get_question_id($question_id);
		$data['courseData'] = $this->courseManager->get_course_id($module_id);
		...
		$this->load->view('eexam/pages/course/create_answer_page',$data);
		return;
	}
...
//------------------------------------------------------------------------------------------------------------------------------------
	...
	public function update_answer_c($questionID, $courseID)
	{
		...
		{
			$answerData =  $this->courseManager->get_answer_id($answer_id);  						// get a answer by id
			
			if (empty($answerData)) 
	        {
	        	if ($data['questionData']['question_category'] == "true_or_false")
	        	{
	        		$theAnswers = $this->courseManager->get_all_question_answers($question_id);  // get  all answer of this course
	        		...
	        		else
	        		{
	        			if ($answer_status == "right")
						{
							...
							$this->courseManager->update_question_answer_status($question_id,"wrong"); 
						}
						elseif ($answer_status == "wrong")
						{
							// update all answers with right
							$this->courseManager->update_question_answer_status($question_id,"right");
						}
	        			// insert answer
						$this->courseManager->insert_answer($answer_title, $answer_status, $question_id); 
			...
			else
			{
				if ($data['questionData']['question_category'] == "true_or_false" )
				{
					...
					//update a answer
					$this->courseManager->update_answer($answer_id,$answer_title,$answer_status); 
			...
			$data['allAnswers'] = $this->courseManager->get_all_question_answers($question_id);  // get  all answer of this course
			if (empty($data['allAnswers']))
			{
				$data['answerError'] = "No Answer Found";
			}
...
//----------------------------------------------------------------------------------------------------------------------------
	...
	public function question_delete_answer_c($answerID, $questionID, $moduleID)
	{
        ...
        {
        	$this->courseManager->delete_answer($answer_id); 							// delete a answer
...
//----------------------------------------------------------------------------------------------------------------------------------------
	...
	public function edit_answer_c($answerID, $questionID, $moduleID)
	{
        ...
        else
        {
        	$data['courseData'] = $this->courseManager->get_course_id($module_id);
        	$data['questionData'] = $this->courseManager->get_question_id($question_id);
        	$data['answerData'] =  $this->courseManager->get_answer_id($answer_id);  		// get a answer by id
